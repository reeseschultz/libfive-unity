...
namespace libfivesharp
{
    ...
    public class LFShape : MonoBehaviour
    {
        ...
        // Subscribe to Camera callbacks for drawing and refresh the mesh OnEnable.
        void OnEnable()
        {
        ...
        void Draw(Camera camera)
        {
            ...
            if (cachedMesh.vertexCount > 0 && material && isRootNode) Graphics.DrawMesh(cachedMesh, Matrix4x4.identity, material, gameObject.layer, camera);
        ...
        public LFTree Evaluate()
        {
            ...
            {
                if (op > 0 && (int)op < 100) tree = evaluateNonary(op);
                ...
                if (isRootNode && tree != null) tree = LFMath.Transform(tree, transform.localToWorldMatrix);
                else tree = LFMath.Transform(tree, transform.parent.worldToLocalMatrix * transform.localToWorldMatrix);
        ...
        {
            if (op == LibFive_Operation.Circle) return LFMath.Circle(0.5f);
            else if (op == LibFive_Operation.Sphere) return LFMath.Sphere(0.5f);
            else if (op == LibFive_Operation.Box) return LFMath.Box(-Vector3.one * 0.5f, Vector3.one * 0.5f);
            else if (op == LibFive_Operation.Cylinder) return LFMath.Cylinder(0.5f, 1f, Vector3.back * 0.5f);
            return tree;
        }
        ...
        LFTree evaluateUnary(LibFive_Operation op, LFTree tree)
        {
            if (tree != null)
            {
                if (op == LibFive_Operation.Transform) return tree;
                else if (op == LibFive_Operation.Inverse) return -tree;
                else if (op == LibFive_Operation.Mirror) return LFMath.SymmetricX(tree);
                else if (op == LibFive_Operation.Shell) return LFMath.Shell(tree, 0.025f);
            }
        ...
        LFTree evaluateNnary(LibFive_Operation op, params LFTree[] trees)
        {
            if (trees.Length > 0 && trees[0] != null)
            {
                ...
                if (op == LibFive_Operation.Union) return LFMath.Union(trees);
                else if (op == LibFive_Operation.Intersection) return LFMath.Intersection(trees);
                else if (op == LibFive_Operation.Difference) return LFMath.Difference(trees);
                else if (op == LibFive_Operation.Blend) return LFMath.Blend(0.1f, trees);
            }
        ...
        bool transformHasChanged()
            => computeLocalTransformHash() != localTransformHash;
    }
